...
(function() {
  var createKeyObject, crypto, cryptoutilsbrowser, hashToScalar, noble, tbut;
  ...
  //###########################################################
  //region shas
  cryptoutilsbrowser.sha256Hex = async function(content) {
  ...
  cryptoutilsbrowser.sha512Hex = async function(content) {
    var contentBytes, hashBytes;
    if ((typeof content) === "string") {
      contentBytes = tbut.utf8ToBytes(content);
    } else {
  ...
  //###########################################################
  cryptoutilsbrowser.sha256Bytes = async function(content) {
    var contentBytes;
    if ((typeof content) === "string") {
      contentBytes = tbut.utf8ToBytes(content);
    } else {
  ...
  cryptoutilsbrowser.sha512Bytes = async function(content) {
    var contentBytes;
    if ((typeof content) === "string") {
      contentBytes = tbut.utf8ToBytes(content);
    } else {
  ...
  //###########################################################
  //region salts
  cryptoutilsbrowser.createRandomLengthSalt = function() {
  ...
  cryptoutilsbrowser.removeSalt = function(content) {
    var char, i, j, len;
  ...
  //###########################################################
  //region encryption
  cryptoutilsbrowser.asymetricEncrypt = async function(content, publicKeyHex) {
  ...
  cryptoutilsbrowser.asymetricDecrypt = async function(secrets, privateKeyHex) {
  ...
  //###########################################################
  cryptoutilsbrowser.symetricEncryptHex = async function(content, keyHex) {
  ...
  cryptoutilsbrowser.symetricDecryptHex = async function(gibbrishHex, keyHex) {
    var aesKeyHex, algorithm, contentBuffer, gibbrishBuffer, ivBuffer, ivHex, key;
    ivHex = keyHex.substring(0, 32);
    aesKeyHex = keyHex.substring(32, 96);
    ivBuffer = tbut.hexToBytes(ivHex);
  ...
  //###########################################################
  //region signatures
  cryptoutilsbrowser.createSignature = async function(content, signingKeyHex) {
    var hashHex;
    hashHex = (await this.sha256Hex(content));
    return (await noble.sign(hashHex, signingKeyHex));
  };

  cryptoutilsbrowser.verify = async function(sigHex, keyHex, content) {
    var hashHex;
