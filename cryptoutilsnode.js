...
(function() {
  var algorithm, crypto, cryptoutilsnode, hashToScalar, noble, sha256, sha512, tbut;
  ...
  //###########################################################
  //region shas
  cryptoutilsnode.sha256Hex = function(content) {
    return tbut.bytesToHex(sha256(content));
  };

  cryptoutilsnode.sha512Hex = function(content) {
  ...
  //###########################################################
  //region salts
  cryptoutilsnode.createRandomLengthSalt = function() {
  ...
  cryptoutilsnode.removeSalt = function(content) {
    var char, i, j, len;
  ...
  //###########################################################
  //region encryption
  cryptoutilsnode.asymetricEncrypt = async function(content, publicKeyHex) {
  ...
  cryptoutilsnode.asymetricDecrypt = async function(secrets, privateKeyHex) {
  ...
  //###########################################################
  cryptoutilsnode.symetricEncryptHex = function(content, keyHex) {
  ...
  cryptoutilsnode.symetricDecryptHex = function(gibbrishHex, keyHex) {
  ...
  //###########################################################
  //region signatures
  cryptoutilsnode.createSignature = async function(content, signingKeyHex) {
    var hashHex;
    hashHex = this.sha256Hex(content);
    return (await noble.sign(hashHex, signingKeyHex));
  };

  cryptoutilsnode.verify = async function(sigHex, keyHex, content) {
    var hashHex;
    hashHex = this.sha256Hex(content);
